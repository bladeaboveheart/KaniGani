...
const updates = JSON.parse(fs.readFileSync('scratch/proposal_58_60.json', 'utf8'));

console.log(`Starting execution for ${updates.length} items (Level 58-60 - GRAND FINALE)...`);
...
// Execute in chunks of 25 items per request to avoid payload limits and timeouts
const CHUNK_SIZE = 25;
let processed = 0;

for (let i = 0; i < updates.length; i += CHUNK_SIZE) {
  const chunk = updates.slice(i, i + CHUNK_SIZE);
  const combinedSql = chunk.map(generateBlock).join('\n');
  
  console.log(`Executing batch ${Math.floor(i / CHUNK_SIZE) + 1}/${Math.ceil(updates.length / CHUNK_SIZE)} (items ${i + 1} to ${Math.min(i + CHUNK_SIZE, updates.length)})...`);
  ...
  if (!res.ok) {
    ...
    console.error(`Error executing batch at index ${i}:`, res.status, errText);
    process.exit(1);
  }
...
console.log(`\n🎉 GRAND FINALE SUCCESS! Successfully executed updates for all ${processed} items in Level 58-60.`);
