...
async function run() {
  ...
  const seedPath = path.join(__dirname, '..', 'sql', 'seed_n1.sql');
  if (!fs.existsSync(seedPath)) {
    console.error('seed_n1.sql not found at ' + seedPath);
    process.exit(1);
  }
  
  console.log('Reading seed_n1.sql...');
  const sqlContent = fs.readFileSync(seedPath, 'utf8');
  console.log(`Loaded seed_n1.sql (${(sqlContent.length / (1024 * 1024)).toFixed(2)} MB).`);
  ...
  console.log('Database seeding completed successfully for JLPT N1!');
}
