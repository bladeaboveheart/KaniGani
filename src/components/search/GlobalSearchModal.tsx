...
export default function GlobalSearchModal({ isOpen, onClose }: GlobalSearchModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Search Bar Input */}
        <div className="flex items-center px-4 py-3.5 border-b border-slate-200 dark:border-slate-800 relative bg-white dark:bg-slate-900">
          ...
          <input
            ...
            className="w-full bg-transparent text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-base sm:text-lg focus:outline-none"
          />
          ...
          <div className="flex items-center space-x-2 shrink-0 ml-2">
            ...
            <kbd className="hidden sm:inline-flex items-center px-2 py-0.5 text-xxs font-bold text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md">
              ESC
            </kbd>
          </div>
        </div>
        ...
        {/* Filter Pills */}
        <div className="flex items-center space-x-1.5 px-4 py-2.5 bg-slate-50/80 dark:bg-slate-950/40 border-b border-slate-200/80 dark:border-slate-800/80 overflow-x-auto text-xs font-semibold">
          <button
            type="button"
        ...
        >
          ...
          {!query.trim() && (
            <div className="py-8 px-4 text-center">
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-950/40 text-indigo-500 dark:text-indigo-400 mb-3">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto mt-1 leading-relaxed">
                ...
                arti bahasa Indonesia / Inggris (misal: <span className="font-semibold text-slate-700 dark:text-slate-300">air</span>, <span className="font-semibold text-slate-700 dark:text-slate-300">matahari</span>),
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                <span className="text-xxs font-bold text-slate-400 uppercase tracking-wider">Contoh cepat:</span>
                {['水', '日', '猫', 'mizu', 'melihat', 'senin'].map((sample) => (
                  <button
                    ...
                    className="px-2 py-1 rounded-md text-xs font-semibold bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 transition-colors"
                  >
                    {sample}
                  </button>
                ))}
          ...
          {results.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center space-x-3 sm:space-x-3.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center space-x-1.5 sm:space-x-2">
                      <span className="text-sm sm:text-base font-bold text-slate-900 dark:text-white capitalize truncate">
                        {item.primary_meaning || item.slug}
                      </span>
                      <span className={`px-1.5 py-0.5 text-xxs font-extrabold rounded uppercase tracking-wider border shrink-0 ${cfg.badgeBg}`}>
                        {cfg.label}
                      </span>
                      <span className="text-xxs font-bold text-slate-400 dark:text-slate-500 shrink-0">
                        Lv. {item.level}
                      </span>
                    </div>

                    {item.primary_reading && (
                      <p className="text-xs text-slate-500 dark:text-slate-400 font-japanese font-medium mt-0.5 truncate">
                        {item.primary_reading}
                      </p>
                    )}
                  </div>
                ...
                <div className="flex items-center space-x-1 text-slate-400 dark:text-slate-500 group-hover:text-slate-700 dark:group-hover:text-slate-200 transition-colors shrink-0 pl-2">
                  <span className="hidden sm:inline text-xxs font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
                    Buka
                  </span>
        ...
        {/* Modal Footer */}
        <div className="flex items-center justify-between px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border-t border-slate-200 dark:border-slate-800 text-xxs text-slate-500 dark:text-slate-400 font-medium">
          <div className="flex items-center space-x-3">
            <span className="flex items-center space-x-1">
              <kbd className="px-1.5 py-0.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded shadow-2xs font-bold">
                ↑
              </kbd>
              <kbd className="px-1.5 py-0.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded shadow-2xs font-bold">
                ↓
            ...
            <span className="flex items-center space-x-1">
              <kbd className="px-1.5 py-0.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded shadow-2xs font-bold flex items-center">
