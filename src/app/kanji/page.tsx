...
export default function KanjiPage() {
  ...
  // Load related vocabularies when a Kanji is clicked/selected
  useEffect(() => {
    async function loadRelatedVocab() {
      if (!selectedItem) {
        setSelectedItemVocabs([]);
        return;
      }
      ...
      try {
        ...
        // Query items where type = 'vocabulary' and prerequisite is this selectedItem.id
        const { data, error } = await supabase
          ...
          `)
          .eq('requires_item_id', selectedItem.id);
  ...
  // Load prerequisite radicals when a Kanji is clicked/selected
  useEffect(() => {
    async function loadPrerequisiteRadicals() {
      if (!selectedItem) {
        setSelectedItemRadicals([]);
        return;
      }
      ...
      try {
        ...
        // Query items where type = 'radical' and this selectedItem requires it
        const { data, error } = await supabase
          .from('item_prerequisites')
          .select(`
            requires_item_id,
        ...
        // Fetch user progress for these radicals
        const { data: progressData } = await supabase
          .from('user_progress')
          .select('item_id, srs_stage, unlocked_at, next_review')
          .eq('user_id', user.id)
        ...
        setSelectedItemRadicals(radicals);
      } catch (err) {
        console.error('Error loading prerequisite radicals:', err);
      } finally {
        setLoadingRadicals(false);
      }
    }
    loadPrerequisiteRadicals();
  }, [selectedItem]);
  ...
  const getSrsColorClass = (stage: number, nextReview?: string | null) => {
    ...
    if (stage === 1 && !nextReview) return 'bg-kanji/5 text-kanji border border-kanji/20 dark:bg-kanji/10';
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-55 text-slate-900 dark:bg-slate-950 dark:text-slate-100 transition-colors duration-300">
            ...
            {hasMore && (
              <div className="flex justify-center pt-6">
                <button
                  ...
                  className="px-6 py-3 bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-700 hover:to-rose-700 text-white text-xs font-bold rounded-2xl shadow-md hover:shadow-lg transition-all duration-200 disabled:opacity-50 flex items-center space-x-2 cursor-pointer"
                >
                  {loadingMore ? (
                    <>
      ...
      {/* SECONDARY VOCABULARY DETAIL MODAL DRAWER OVERLAY */}
      {selectedVocabDetail && (
        <div
          onClick={() => setSelectedVocabDetail(null)}
          className="fixed inset-0 z-[60] overflow-y-auto bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 transition-all duration-300"
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden flex flex-col animate-fade-in max-h-[90vh]"
      ...
      {/* SECONDARY RADICAL DETAIL MODAL DRAWER OVERLAY */}
