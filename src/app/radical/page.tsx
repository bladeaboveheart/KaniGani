...
export default function RadicalPage() {
  const {
    ...
    closeItemDetail,
    detailLoading,
  } = useDictionaryItems('radical');

  const theme = ITEM_THEMES.radical;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="font-semibold text-sm">Menyiapkan Pustaka Radikal WaniKani...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-55 text-slate-900 dark:bg-slate-950 dark:text-slate-100 transition-colors duration-300">
      ...
      <WaniKaniSubNav currentType="radical" counts={{ radical: items.length }} />
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 animate-fade-in">
        ...
        <section className={`bg-gradient-to-r ${theme.bannerGradient} text-white p-6 sm:p-8 rounded-3xl shadow-md relative overflow-hidden`}>
          ...
          <div className="relative z-10 space-y-1.5">
            ...
            <p className="text-xs sm:text-sm text-cyan-100 max-w-2xl leading-relaxed">
              {theme.subtitle} Radikal adalah fondasi utama yang menyusun seluruh kanji di KaniGani dan WaniKani.
            </p>
          </div>
        </section>
        ...
        {/* Filter, Search & Display Toggle Bar */}
        <DictionaryFilterBar
        ...
        {/* Radicals Grouped by Level Layout */}
        <LevelGroupSection
          allItems={items}
